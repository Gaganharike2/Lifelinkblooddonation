...
router.post('/register', async (req, res) => {
  ...
  const otp = await sendAuthOtp({ id: result.insertId, email, mobile }, 'register', channel);
  res.status(201).json(otpResponse({ message: `Registered. Verify OTP sent to your ${channel === 'phone' ? 'phone' : 'email'}.`, userId: result.insertId, channel }, otp));
});
...
router.post('/login', async (req, res) => {
  ...
  if (!user.is_verified) {
    const channel = otpChannel(req);
    const otp = await sendAuthOtp(user, 'login', channel);
    return res.status(202).json(otpResponse({ message: `OTP verification required. Code sent to your ${channel === 'phone' ? 'phone' : 'email'}.`, userId: user.id, needsOtp: true, channel }, otp));
  }
...
router.post('/resend-otp', async (req, res) => {
  ...
  const channel = otpChannel(req);
  const otp = await sendAuthOtp(user, purpose, channel);
  res.json(otpResponse({ message: `OTP sent to your ${channel === 'phone' ? 'phone' : 'email'}.`, channel }, otp));
});
...
router.post('/forgot-password', async (req, res) => {
  ...
  const user = rows[0];
  const channel = otpChannel(req);
  const otp = await sendAuthOtp(user, 'reset', channel);
  res.json(otpResponse({ message: `If the account exists, a reset OTP has been sent to the registered ${channel === 'phone' ? 'phone' : 'email'}.`, channel }, otp));
});
